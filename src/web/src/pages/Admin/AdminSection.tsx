...
export default function AdminSection() {
  ...
  const createMut = useMutation({ mutationFn: () => createSection(title, type, { articleIds: [], limit: 5 }), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-sections'] }) })
  const deleteMut = useMutation({ mutationFn: deleteSection, onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-sections'] }) })
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-[#111827] mb-6">板块管理</h1>
      ...
      <div className="bg-white rounded-lg border border-[#E5E7EB] p-4 mb-6 flex items-center gap-3">
        ...
        <select value={type} onChange={e => setType(e.target.value)} className="border border-[#E5E7EB] rounded-md px-3 py-2 text-sm">
          <option value="article_list">文章列表</option>
          <option value="skill_leaderboard">Skill 榜单</option>
          <option value="external_link">外链</option>
        </select>
        <button onClick={() => { if (title.trim()) { createMut.mutate(); setTitle('') } }} disabled={createMut.isPending} className="px-4 py-2 bg-[#4F46E5] text-white rounded-md text-sm hover:bg-[#4338CA] disabled:opacity-50">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {sections.map((s: any) => (
            <div key={s._id} className="bg-white rounded-lg border border-[#E5E7EB] p-4 flex items-center gap-4">
              <div className="flex-1">
                <p className="font-medium text-[#111827]">{s.title}</p>
                <p className="text-xs text-[#9CA3AF] mt-0.5">ID: {s._id} · {typeTag(s.type)} · {s.visibility}</p>
              </div>
